...
class Client extends EventEmitter {
   ...
   */
  syncGuilds(guilds) { guilds = guilds || this.guilds.array();
    if (!this.user.bot) {
      this.ws.send({
        op: 12,
        d: guilds.map(g => g.id),
   ...
   */
  sweepMessages(lifetime) { lifetime = lifetime || this.options.message_cache_lifetime;
  ...
  setTimeout(fn$jscomp$0) {
	  var _this$jscomp$0 = this;
	  var _len$jscomp$0 = arguments.length;
	  var params$jscomp$1 = Array(_len$jscomp$0 > 1 ? _len$jscomp$0 - 1 : 0);
	  var _key$jscomp$0 = 1;
	  for (; _key$jscomp$0 < _len$jscomp$0; _key$jscomp$0++) {
		params$jscomp$1[_key$jscomp$0 - 1] = arguments[_key$jscomp$0];
	  }
	  var timeout$jscomp$1 = setTimeout.apply(undefined, [function() {
		fn$jscomp$0();
		_this$jscomp$0._timeouts.delete(timeout$jscomp$1);
	  }].concat(params$jscomp$1));
  ...
  setInterval() {
	  var interval$jscomp$0 = setInterval.apply(undefined, arguments);
